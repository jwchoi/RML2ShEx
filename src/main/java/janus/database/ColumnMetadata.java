...
class ColumnMetadata implements Comparable<ColumnMetadata> {
	...
	Optional<Set<String>> getValueSet(String catalog, String table) {
		if (valueSet == null)
			valueSet = Shaper.dbBridge.getValueSet(catalog, table, columnName);
	...
	boolean isUnsigned(String catalog, String table) {
		if (isUnsigned == null)
			isUnsigned = Optional.of(Shaper.dbBridge.isUnsigned(catalog, table, columnName));
	...
	Optional<Integer> getNumericScale(String catalog, String table) {
		if (numericScale == null)
			numericScale = Shaper.dbBridge.getNumericScale(catalog, table, columnName);
	...
	Optional<Integer> getNumericPrecision(String catalog, String table) {
		if (numericPrecision == null)
			numericPrecision = Shaper.dbBridge.getNumericPrecision(catalog, table, columnName);
	...
	Optional<Integer> getCharacterMaximumLength(String catalog, String table) {
		if (characterMaximumLength == null)
			characterMaximumLength = Shaper.dbBridge.getCharacterMaximumLength(catalog, table, columnName);
	...
	Optional<Integer> getMaximumOctetLength(String catalog, String table) {
		if (maximumOctetLength == null)
			maximumOctetLength = Shaper.dbBridge.getCharacterOctetLength(catalog, table, columnName);
	...
	Optional<String> getMaximumIntegerValue(String catalog, String table) {
		if (maximumIntegerValue == null)
			maximumIntegerValue = Shaper.dbBridge.getMaximumIntegerValue(catalog, table, columnName);
