...
class XMLDataSource extends DataSource {
    ...
    XMLDataSource(Session session, Dataset<Row> df) {
        super(session, df);
        this.df = df;
    }
    ...
    void acquireMinAndMaxValue(Column column) {
        ...
        String prefix = column.getName().substring(0, column.getName().indexOf("/")+1);
        ...
        List<Row> rows = selectColsDF.summary("min", "max").select("summary", column.getName()).collectAsList();
        for (Row row : rows) {
    ...
    void acquireMinAndMaxLength(Column column) {
        ...
        org.apache.spark.sql.Column[] selectCols = Arrays.stream(selectColsAsString).map(df::col).toArray(org.apache.spark.sql.Column[]::new);

        String newColumn = column.getName() + "_length";
        for (int i = 0; Arrays.asList(selectColsAsString).contains(newColumn); i++) newColumn += i;

        Dataset<Row> lengthAddedDF = df.select(selectCols).withColumn(newColumn, functions.length(df.col(column.getName())));

        List<Row> rows = lengthAddedDF.summary("min", "max").select("summary", newColumn).collectAsList();
        for (Row row : rows) {
            String key = row.getString(0);
            String value = row.getString(1);
    ...
    private boolean isNestedColumn(Column column) { return column.getName().indexOf("/") == -1; }
}
